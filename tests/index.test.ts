...
describe('RealityDefender SDK', () => {
  ...
  const mockGetDetectionResult = getDetectionResult as jest.MockedFunction<typeof getDetectionResult>;
  ...
  describe('constructor', () => {
    it('should throw error when API key is missing', () => {
      expect(() => new RealityDefender({ apiKey: '' }))
  ...
  describe('upload', () => {
    ...
    it('should pass through RealityDefenderError errors', async () => {
      ...
      await expect(sdk.upload(uploadOptions))
    ...
    it('should wrap generic errors in RealityDefenderError', async () => {
      ...
      await expect(sdk.upload(uploadOptions))
        .rejects.toThrow(new RealityDefenderError('Upload failed: Unknown upload error', 'upload_failed'));
    });
  });
  ...
  describe('getResult', () => {
    ...
    it('should handle not found errors', async () => {
      ...
      const notFoundError = new RealityDefenderError('Result not found for request ID: non-existent-id', 'not_found');
      mockGetDetectionResult.mockRejectedValueOnce(notFoundError);
      
      await expect(sdk.getResult('non-existent-id'))
    ...
    it('should handle server errors in getResult', async () => {
      ...
      await expect(sdk.getResult('request-123'))
    ...
    it('should handle generic errors in getResult', async () => {
      ...
      const genericError = new RealityDefenderError('Failed to get result: Generic Error', 'unknown_error');
  ...
  describe('internal polling', () => {
    ...
    let originalSleep: typeof import('../src/utils/async').sleep;
